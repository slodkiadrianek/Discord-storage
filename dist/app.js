...
import { client, BOT_TOKEN, createThreadInChannel, fetchData, } from "./model/discord.js";
...
app.post("/auth/pdf", upload.single("pdf"), async (req, res) => {
    const file = req.file;
...
app.post("/auth/txt", upload.single("txt"), async (req, res) => {
    const file = req.file;
    const filename = file.filename;
    const name = filename.split(".")[0];
    const __dirname = path.resolve();
...
app.post("/auth/txt/concat", upload.array("txt", 30), async (req, res) => {
...
app.post("/auth/pdf/concat", upload.array("pdf", 30), async (req, res) => {
    const files = req.files;
    const filePaths = files.map((file) => file.path);
    console.log(filePaths);
    const name = "concat";
...
app.post("/auth/fetch", async (req, res) => {
